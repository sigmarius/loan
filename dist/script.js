...
/* harmony import */ var _modules_show_block__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./modules/show-block */ "./src/js/modules/show-block.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  feedSlider.init();
  new _modules_video__WEBPACK_IMPORTED_MODULE_2__["default"]('.showup .play', '.overlay').init();
  ...
  new _modules_show_block__WEBPACK_IMPORTED_MODULE_5__["default"]('.plus').init();
});
...
function () {
  function Difference(oldOfficer, newOfficer, items) {
    ...
    try {
      ...
      console.log("Something is wrong :( ".concat(evt));
    }
  }
...
function () {
  ...
  _createClass(Form, [{
    ...
    key: "validateEmailInputs",
    value: function validateEmailInputs() {
    ...
    value: function initMask() {
      ...
      function createMask(evt) {
        ...
        def = matrix.replace(/\D/g, ''),
            val = this.value.replace(/\D/g, ''); // не даст пользователю удалить штатные элементы маски +, 1 и др, заданные в matrix    
...
/*#__PURE__*/
function () {
  function ShowBlock(triggers) {
    _classCallCheck(this, ShowBlock);

    this.btns = document.querySelectorAll(triggers);
  }

  _createClass(ShowBlock, [{
    key: "init",
    value: function init() {
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== "BUTTON") {
            ...
            var active = _this2.slides[i]; // помещаем его перед первым слайдом
...
var Slider = function Slider() {
  ...
  this.autoplay = autoplay; // определяет текущий слайд
...
/*#__PURE__*/
function () {
  function Video(triggers, overlay) {
    _classCallCheck(this, Video);

    this.btns = document.querySelectorAll(triggers);
    this.overlay = document.querySelector(overlay);
    this.close = this.overlay.querySelector('.close'); // привязываем контекст состояния видео к экземпляру класса во избежание потери окружения
  ...
  _createClass(Video, [{
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn, i) {
        try {
          ...
          var blockedElem = btn.closest('.module__video-item').nextElementSibling; // получаем каждый второй элемент
    ...
    value: function onPlayerStateChange(state) {
      try {
        // получаем первого родителя нажатой кнопки => нажодим соседний блок с видео
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // копируем иконку Play => разместим ее вместо иконки замка

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // из документации API Youtube => state.data=0 видео завершено
